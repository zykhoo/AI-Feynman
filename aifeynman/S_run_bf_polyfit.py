...
def run_bf_polyfit(pathdir,pathdir_transformed,filename,pathdir_original,filename_original,BF_try_time,BF_ops_file_type, PA, polyfit_deg=3, output_type=""):
    input_data = np.loadtxt(pathdir_transformed+filename)
    compute_loss_data = np.loadtxt(pathdir_original+filename_original)
#############################################################################################################################
    if np.isnan(input_data).any()==False:
        ...
        try:
            ...
            for i in range(len(complexity)):
                ...
                print("1A added ", complexity[i], errors[i], str(eqns[i]))
            ...
            for i in range(len(express)):
                try:
                    ...
                    PA.add(Point(x=bf_gd_update[1],y=get_symbolic_expr_error(compute_loss_data,str(bf_gd_update[2])),data=bf_gd_update[2]))
                    print("1B added ", bf_gd_update[1], get_symbolic_expr_error(compute_loss_data,str(bf_gd_update[2])), str(bf_gd_update[2]))
                except:
                    continue
        except:
            pass
    ...
    #############################################################################################################################
        ...
        try:
            ...
            for i in range(len(complexity)):
                PA.add(Point(x=complexity[i], y=errors[i], data=eqns[i]))
                print("2A added ", complexity[i], errors[i], str(eqns[i]))
            ...
            for i in range(len(express)):
                try:
                    ...
                    # PA.add(Point(x=bf_gd_update[1],y=bf_gd_update[0],data=bf_gd_update[2]))
                    PA.add(Point(x=bf_gd_update[1], y=get_symbolic_expr_error(compute_loss_data,str(bf_gd_update[2])),data=bf_gd_update[2]))
                    print("2B added ", bf_gd_update[1], get_symbolic_expr_error(compute_loss_data,str(bf_gd_update[2])), str(bf_gd_update[2]))
                except:
                    continue
        except:
            pass
    ...
    #############################################################################################################################
        ...
        try:
            ...
            print("3A added ", complexity, polyfit_err, str(eqn))
            ...
            for l in range(len(PA_poly.get_pareto_points())):
                ...
                print("3B added ", PA_poly.get_pareto_points()[l][0], PA_poly.get_pareto_points()[l][1], PA_poly.get_pareto_points()[l][2])
